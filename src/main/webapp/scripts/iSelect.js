(function() {
	...
	function IosSelect(level, data, options) {
		...
		this.callback = options.selectcallback;
		this.cancelcallback = options.cancelcallback;
		this.title = options.title || '';
		this.itemHeight = options.itemHeight || 35;
		this.headerHeight = options.headerHeight || 44;
		this.id = options.idKey || 'id';
		this.value = options.valueKey || 'value';
		this.parentId = options.parentIdKey || 'parentId';
		this.init();
	};
	...
	IosSelect.prototype = {
		...
		initLayer: function() {
			...
			this.selectBtnCancel = this.iosSelectLayer.el.querySelector('.close');
			this.selectBtnCancel.addEventListener('click', function(e) {
		...
		setOneLevel: function(oneLevelId, twoLevelId, threeLevelId) {
			...
			if (!oneLevelId) {
				oneLevelId = oneLevelData[0][self.id];
			}
			var oneHtml = '';
			var atIndex = 0;
			oneHtml += '<li></li>';
			...
			oneLevelData.forEach(function(v, i, o) {
				if (v[self.id]  === oneLevelId) {
					oneHtml += '<li ' + iosSelectUtil.attrToHtml(v) + ' class="at">' + v[self.value] + '</li>';
					atIndex = i + 1 + 3;
				} else {
					oneHtml += '<li ' + iosSelectUtil.attrToHtml(v) + '>' + v[self.value] + '</li>';
				}
			});
			oneHtml += '<li></li>';
			oneHtml += '<li></li>';
		...
		getTwoLevel: function(oneLevelId) {
			...
			if (this.options.oneTwoRelation === 1) {
				...
				if (!hasValue) {
					var v = {id: '-1784917893179', value: '', parentId: oneLevelId};
					twoLevelData.push(v);
				}
			} else {
		...
		setTwoLevel: function(oneLevelId, twoLevelId, threeLevelId) {
			...
			var atIndex = 0;
			if (!twoLevelId) {
				twoLevelId = twoLevelData[0][self.id];
			}
			var twoHtml = '';
			twoHtml += '<li></li>';
			twoHtml += '<li></li>';
			...
			twoLevelData.forEach(function(v, i, o) {
				if (v[self.id] === twoLevelId) {
					twoHtml += '<li ' + iosSelectUtil.attrToHtml(v) + ' class="at">' + v[self.value] + '</li>';
					atIndex = i + 1 + 3;
				} else {
					twoHtml += '<li ' + iosSelectUtil.attrToHtml(v) + '>' + v[self.value] + '</li>';
				}
			});
			twoHtml += '<li></li>';
			twoHtml += '<li></li>';
		...
		getThreeLevel: function(twoLevelId) {
			...
			if (this.options.twoThreeRelation === 1) {
				...
				if (!hasValue) {
					var v = {id: '-17849178931793292', value: '', parentId: twoLevelId};
					threeLevelData.push(v);
				}
			} else {
		...
		setThreeLevel: function(oneLevelId, twoLevelId, threeLevelId) {
			...
			var atIndex = 0;
			if (!threeLevelId) {
				threeLevelId = threeLevelData[0][self.id];
			}
			var threeHtml = '';
			threeHtml += '<li></li>';
			threeHtml += '<li></li>';
			...
			threeLevelData.forEach(function(v, i, o) {
				if (v[self.id] === threeLevelId) {
					threeHtml += '<li ' + iosSelectUtil.attrToHtml(v) + ' class="at">' + v[self.value] + '</li>';
					atIndex = i + 1 + 3;
				} else {
					threeHtml += '<li ' + iosSelectUtil.attrToHtml(v) + '>' + v[self.value] + '</li>';
				}
			});
			threeHtml += '<li></li>';
			threeHtml += '<li></li>';
